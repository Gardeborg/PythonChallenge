wall_of_text = """kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
#Not very nice, brute-force
def foo(aString):
	result = ''
	for i in range(4,len(aString)-4):
		if aString[i].islower():
			if aString[i-1].isupper() and \
			aString[i-2].isupper() and \
			aString[i-3].isupper() and \
			aString[i-4].islower() and \
			aString[i+1].isupper() and \
			aString[i+2].isupper() and \
			aString[i+3].isupper() and \
			aString[i+4].islower():
				result += aString[i]
	return result
print(foo(wall_of_text))
